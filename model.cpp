...
{
	if(!draughts::model::model::player_exists(const std::string& p))
	{
...
{
	return find(player_vector.begin(), player_vector.end(), pname) != player_vector.end());
}
    ...
    const
{
    ...
    for (auto it = player_vector.begin(); it!= get_player_list.end(); ++it)
		namelist.insert(std::make_pair(*it.get_player_ID(),*it.get_player_name()));
